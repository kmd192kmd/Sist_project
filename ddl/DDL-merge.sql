...
create table ATTENDANCE_TYPE(
    ...
    --constraint attendanceTypeSeq_limit check (attendanceTypeSeq in (1, 2, 3, 4, 5, 6)) --> 추후 상태추가를 위해 삭제
    constraint attendance_type_uq unique (attendanceTypeState) -- 새로추가
);
...
create table POINT_VALUE(
    ...
    CONSTRAINT chk_point_total_100 CHECK (pointValueAttendance + pointValueWrittenTest + pointValuePracticalTest = 100)
);
